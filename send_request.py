    ...
    if user_info['coins'] < int(x):
        ...
        if ask2.text and "t.me" in ask2.text:
            ...
            try:
                ...
                return
            try:
                z = Co(name='ww', api_id=19312827, api_hash='84da7f08e87849853b2fa6728e4192a2', session_string=e, workers=20)
                await z.start()
...
async def reaction_s(app, query):
    ...
    if ask.text:
        ...
        if int(x) > int(user_info['coins']):
            await ask1.reply(f"مامعك سعر الريأكشنات تحتاج: {x} دولار .")
            return
        if count <1:
